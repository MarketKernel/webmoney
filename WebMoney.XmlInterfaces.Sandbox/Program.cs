...
namespace WebMoney.XmlInterfaces.Sandbox
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            //var initializer = new Initializer(program._primaryWmId, program._primaryKeeperKey);
            //initializer.Apply();
            ...
            try
            {
                program.MerchantOperationFilterTest();
            }
            catch (Exception exception)
            {
                Console.WriteLine(exception);
        ...
        // X18. Детали операции через merchant
        public void MerchantOperationFilterTest()
        {
            //var transferFilter = new TransferFilter(_primaryPurse, DateTime.Now.AddMonths(-17), DateTime.Now.AddMonths(-16));
            //var transferRegister = transferFilter.Submit();
            
            var merchantOperationFilter = new MerchantOperationFilter(_storePurse, 0);
